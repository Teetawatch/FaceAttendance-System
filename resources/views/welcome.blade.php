...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-3xl font-bold text-slate-800 tracking-tight">สวัสดี, {{ Auth::user()->name }} </h2>
            <p class="text-slate-500 mt-1">ยินดีต้อนรับกลับสู่ระบบจัดการเวลาเข้างาน</p>
        </div>
        <div class="flex items-center gap-3">
            <span class="text-sm font-medium text-slate-500 bg-white px-4 py-2 rounded-full shadow-sm border border-slate-100">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] hover:shadow-[0_8px_30px_-4px_rgba(6,81,237,0.1)] transition-all duration-300 group">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-400">พนักงานทั้งหมด</p>
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-[0_2px_10px_-3px_rgba(16,185,129,0.1)] hover:shadow-[0_8px_30px_-4px_rgba(16,185,129,0.1)] transition-all duration-300 group">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-400">มาทำงานวันนี้</p>
            ...
            <div class="mt-4 flex items-center gap-2">
                ...
                <span class="text-xs text-slate-400">จากเมื่อวาน</span>
            </div>
        </div>

        <!-- Card 3: Late Arrivals -->
        <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-[0_2px_10px_-3px_rgba(245,158,11,0.1)] hover:shadow-[0_8px_30px_-4px_rgba(245,158,11,0.1)] transition-all duration-300 group">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-400">มาสาย</p>
                    <h3 class="text-3xl font-bold text-slate-800 mt-2">{{ $lateToday }}</h3>
                </div>
        ...
        <!-- Card 4: Devices -->
        <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-[0_2px_10px_-3px_rgba(139,92,246,0.1)] hover:shadow-[0_8px_30px_-4px_rgba(139,92,246,0.1)] transition-all duration-300 group">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-400">อุปกรณ์ออนไลน์</p>
                    <h3 class="text-3xl font-bold text-slate-800 mt-2">{{ $activeDevices }}<span class="text-lg text-slate-300 font-normal">/{{ $totalDevices }}</span></h3>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-50 flex items-center justify-between">
                <h3 class="font-bold text-slate-800 text-lg">การสแกนล่าสุด</h3>
            ...
            <div class="divide-y divide-slate-50">
                ...
                <div class="px-6 py-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors group">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center overflow-hidden border-2 border-white shadow-sm group-hover:border-primary-100 transition-colors">
                        ...
                        <div>
                            <p class="font-bold text-slate-700 group-hover:text-primary-600 transition-colors">
                                {{ $scan->employee ? $scan->employee->first_name . ' ' . $scan->employee->last_name : 'Unknown' }}
                            </p>
                            <div class="flex items-center gap-2 mt-0.5">
                                <span class="text-xs text-slate-400 bg-slate-50 px-2 py-0.5 rounded border border-slate-100">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-sm font-medium text-slate-500 mt-1 font-mono">
                ...
                <div class="px-6 py-12 text-center">
                    ...
                    <p class="text-slate-500 font-medium">ยังไม่มีข้อมูลการสแกน</p>
                    <p class="text-slate-400 text-sm mt-1">ข้อมูลการเข้างานวันนี้จะแสดงที่นี่</p>
                </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div>
                <h3 class="font-bold text-slate-800 mb-4 px-1">เมนูด่วน</h3>
                <div class="grid grid-cols-2 gap-4">
                    <a href="{{ route('monitor') }}" class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 text-center group">
                        ...
                        <span class="text-sm font-bold text-slate-700">จอภาพ</span>
                    </a>
                    <a href="{{ route('employees.index') }}" class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 text-center group">
                        ...
                        <span class="text-sm font-bold text-slate-700">เพิ่มพนักงาน</span>
                    </a>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                <h3 class="font-bold text-slate-800 mb-4">สถานะระบบ</h3>
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-sm font-medium text-slate-600">WebSocket</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
