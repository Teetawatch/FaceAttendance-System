    ...
    <form method="post" action="{{ route('profile.signature') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        ...
        <div>
            ...
            @if($user->signature_path)
                <div class="mt-2 mb-4">
                    <img src="{{ asset('storage/' . $user->signature_path) }}" alt="Current Signature" class="h-24 border border-primary-100/60 rounded-xl p-1">
                </div>
            @endif

            <input id="signature" name="signature" type="file" accept="image/png, image/jpeg" class="mt-1 block w-full text-sm text-muted
