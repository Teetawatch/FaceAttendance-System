    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <!-- Stat 1 -->
                <div
                    class="bg-card rounded-2xl p-6 shadow-md border border-slate-200/60 hover:shadow-lg hover:-translate-y-1 transition-all cursor-pointer group">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-card border border-slate-200 rounded-2xl shadow-md p-6">
                    <div class="flex items-center justify-between mb-6">
                        <h4 class="text-lg font-bold font-mono text-text">Attendance Trend</h4>
                        <div class="flex gap-2">
                            <span class="w-3 h-3 rounded-full bg-indigo-50/500 mt-1"></span>
                    ...
                    <div class="h-64 w-full flex items-end justify-between relative pt-8 pb-4">
                        ...
                        <div class="absolute inset-0 flex flex-col justify-between pt-8 pb-8 z-0">
                            ...
                            <div class="border-b border-slate-200/50 w-full"></div>
                            <div class="border-b border-slate-200/50 w-full"></div>
                            <div class="border-b border-slate-200/60 w-full"></div>
                        </div>
                        <!-- Bars -->
                        @foreach([60, 80, 75, 95, 85, 100, 92] as $val)
                            <div class="w-8 md:w-12 flex flex-col items-center gap-2 z-10 group cursor-pointer">
                                <div
                                    class="w-full bg-indigo-50/50 rounded-t-lg relative flex flex-col justify-end h-48 overflow-hidden group-hover:bg-primary-100 transition-colors">
                                    <div class="w-full bg-gradient-to-t from-primary-600 to-primary-400 rounded-t-sm transition-all duration-300 group-hover:bg-indigo-50/500"
                                        style="height: {{ $val }}%"></div>
                                </div>
                                <span
                                    class="text-xs font-mono font-medium text-primary-400 group-hover:text-indigo-600 transition-colors">{{ ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][$loop->index] }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-card border border-slate-200 rounded-2xl shadow-md p-6 flex flex-col">
                    <div class="flex items-center justify-between mb-6">
                    ...
                    <div class="flex flex-col gap-4 flex-1 overflow-y-auto pr-2">
                        @foreach(['Sarawut S.', 'Piyapong N.', 'Nattamon T.', 'Supachai P.'] as $name)
                            <div
                                class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200/60 hover:border-primary-200 transition-colors cursor-pointer">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 rounded-full bg-primary-100 text-indigo-600 flex items-center justify-center font-bold text-sm">
                                        {{ substr($name, 0, 1) }}
                                    </div>
                                    <div>
                    ...
                    <button
                        class="w-full mt-4 py-2.5 bg-indigo-50/50 hover:bg-primary-100 text-primary-700 font-semibold rounded-xl transition-colors text-sm border border-slate-200 border-dashed">
                        View All Scans
                    </button>
                </div>
            </div>
