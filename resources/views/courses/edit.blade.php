...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-text font-bold font-mono font-mono">✏️ แก้ไขหลักสูตร</h2>
            <p class="text-primary-600/70 text-sm">{{ $course->name }}</p>
        </div>
    </div>
        ...
        <form action="{{ route('courses.update', $course) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $course->name) }}" required
                       class="w-full px-4 py-3 rounded-xl border border-primary-100 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all @error('name') border-rose-300 @enderror">
                @error('name')
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full px-4 py-3 rounded-xl border border-primary-100 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all">{{ old('description', $course->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date', $course->start_date->format('Y-m-d')) }}" required
                           class="w-full px-4 py-3 rounded-xl border border-primary-100 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all @error('start_date') border-rose-300 @enderror">
                    @error('start_date')
                        <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date', $course->end_date->format('Y-m-d')) }}" required
                           class="w-full px-4 py-3 rounded-xl border border-primary-100 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all @error('end_date') border-rose-300 @enderror">
                    @error('end_date')
                        <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
