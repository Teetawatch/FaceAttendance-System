...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-primary-600/70 text-sm">จัดการจุดลงเวลาและอุปกรณ์ Kiosk ทั้งหมด</p>
        </div>
        <a href="{{ route('devices.create') }}" class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-xl transition-all shadow-sm hover:shadow-md text-sm font-medium">
            ...
            <table class="w-full text-left text-sm text-text/80">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-background/80 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <span class="font-mono text-primary-600/70 bg-background px-2 py-1 rounded text-xs border border-primary-50 select-all">{{ $device->device_code }}</span>
                        </td>
                        ...
                        <td class="px-6 py-4 font-mono text-xs text-primary-600/70">{{ $device->ip_address ?? '-' }}</td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('devices.edit', $device) }}" class="w-8 h-8 flex items-center justify-center rounded-lg text-primary-400 hover:text-primary-600 hover:bg-primary-50 transition-all" title="แก้ไข">
