...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">อุปกรณ์ลงเวลา</h2>
            <p class="text-slate-500 text-sm">จัดการจุดลงเวลาและอุปกรณ์ Kiosk ทั้งหมด</p>
        </div>
        <a href="{{ route('devices.create') }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl transition-all shadow-sm hover:shadow-md text-sm font-medium">
            <i class="fa-solid fa-plus"></i> ลงทะเบียนอุปกรณ์
        </a>
    </div>
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="font-bold text-slate-700 group-hover:text-primary-700 transition-colors">{{ $device->name }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="font-mono text-slate-500 bg-slate-50 px-2 py-1 rounded text-xs border border-slate-100 select-all">{{ $device->device_code }}</span>
                        </td>
                        ...
                        <td class="px-6 py-4 font-mono text-xs text-slate-500">{{ $device->ip_address ?? '-' }}</td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('devices.edit', $device) }}" class="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-primary-600 hover:bg-primary-50 transition-all" title="แก้ไข">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center text-slate-400">
                            <div class="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                <i class="fa-solid fa-tablet-button text-2xl text-slate-300"></i>
                            </div>
                            <p class="font-medium">ไม่พบข้อมูลอุปกรณ์</p>
                            <p class="text-sm mt-1 text-slate-400">เริ่มต้นด้วยการลงทะเบียนอุปกรณ์ใหม่</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
